...
const getSiteHtmlTool = mcpServer.registerTool(
  "getSiteHtmlTool",

  {
    description: "Use this tool to get site's html",
    inputSchema: {
      url: z.url(),
    },
  },
  async ({ url }) => {
    ...
    if (!result.ok) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `failed to get html of site , error : ${error}`,
          },
        ],
        isError: true,
      };
    ...
    return {
      content: [
        {
          type: "text",
          text: `here is html of site : ${html}`,
        },
      ],
    };
  },
...
const getUrlToSaveCss = mcpServer.registerTool(
  "getUrlToSaveCss",
...
const saveCssTool = mcpServer.registerTool(
  "saveCss",
  {
    description:
      "Save a custom CSS file URL to a site's customCssLink field. The CSS URL must match the generated presigned upload URL format.",
    inputSchema: {
      siteUrl: z.url(),
      cssUrl: z
        .string()
...
export { mcpServer, getSiteHtmlTool, getUrlToSaveCss, saveCssTool };
