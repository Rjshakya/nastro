...
export function CreateTemplateDialog({ onSuccess }: CreateTemplateDialogProps) {
  ...
  const handleCreate = async () => {
    if (!input.templateName.trim() || !input.templateUrl.trim()) return;
  ...
  const isValid = input.templateName.trim().length > 0 && input.templateUrl.trim().length > 0;

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button className={"dark:border-border"} size="sm" variant="outline">
            ...
            <div className="mb-3 grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setInput({
                    ...input,
                    templateThumbnailUrl: e.target.value || null,
                  })
                }
              />
            </div>
