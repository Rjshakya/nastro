...
app.get("/tasks", async (c) => {
  const { rows, nextCursor } = await notionDb
    .select()
    .from(tasksTable)
    .execute();
...
app.post("/tasks", async (c) => {
  try {
    ...
    return c.json(
      { error: "Failed to create task", cause: error },
      e.status ?? 500,
    );
  }
});
